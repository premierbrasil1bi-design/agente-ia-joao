...
export class WahaProvider extends ChannelProvider {
  ...
  async getStatus(channel = this.channel) {
    ...
    const rawStatus = raw?.status ?? raw?.state ?? raw?.instance?.state ?? raw?.sessionStatusCanonical?.rawStatus ?? null;
    return normalizeChannelStatus('waha', rawStatus);
  }
