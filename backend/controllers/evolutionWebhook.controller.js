...
export async function handleEvolutionWhatsApp(req, res) {
  try {
    ...
    if (!instance) {
      console.warn('[evolutionWebhook] POST /webhooks/whatsapp/evolution: instance ausente no payload');
      return res.status(200).json({ received: true });
    }

    const channel = await channelsRepo.findByExternalId(instance);
    if (!channel || !channel.agent_id) {
      console.warn('[evolutionWebhook] Canal não encontrado para external_id:', instance);
      return res.status(200).json({ received: true });
    }
    ...
    enqueueConversationTask(conversationKey, async () => {
      ...
      if (result?.replyText && instance) {
        try {
          ...
          console.log('[evolutionWebhook] Resposta enviada para', number);
        } catch (sendErr) {
          console.error('[evolutionWebhook] sendText error:', sendErr.message);
        }
      }
    });
    ...
    console.error('[evolutionWebhook] handleEvolutionWhatsApp:', err);
