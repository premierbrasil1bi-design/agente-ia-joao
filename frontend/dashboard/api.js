...
export const api = {
  ...
  getAgents: (clientId) => request(`/api/agent/dashboard/agents${clientId ? `?client_id=${clientId}` : ''}`),
  getChannels: (agentId) => request(`/api/agent/dashboard/channels${agentId ? `?agent_id=${agentId}` : ''}`),
  getCosts: (params) => {
  ...
  getPrompts: (agentId) => request(`/api/agent/dashboard/prompts?agent_id=${agentId}`),
  getClients: () => request('/api/agent/dashboard/clients'),
};
